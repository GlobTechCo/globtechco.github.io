...
def fetch_rss_items(feed_url):
    ...
    for item in root.findall(".//item"):
        ...
        if not image:
            ...
            if media_el is None:
                media_el = item.find("media:thumbnail", ns)
            if media_el is not None:
                image = media_el.get("url") or ""
        if not image and description_raw:
...
def update_aggregator(all_items):
    ...
    with open(AGGREGATOR_FILE, "w", encoding="utf-8") as f:
        for item in deduped:
            ...
            f.write(f"  image: {json.dumps(item.get('image', ''))}\n")
